...
// ============ 图标配置 ============
const sectionIcons = [
  { shape: 'CHEVRON',        color: 'E8893C', bg: 'FFF3EB' },
  { shape: 'HEXAGON',        color: '4A90D9', bg: 'EBF3FC' },
  { shape: 'CUBE',           color: '27AE60', bg: 'E8F8F0' },
  { shape: 'DIAMOND',        color: '4A90D9', bg: 'EBF3FC' },
  { shape: 'LIGHTNING_BOLT', color: 'E8893C', bg: 'FFF3EB' },
  { shape: 'GEAR_6',         color: 'F2994A', bg: 'FFF3EB' },
  { shape: 'GEAR_9',         color: '4A90D9', bg: 'EBF3FC' },
  { shape: 'STAR_5_POINT',   color: '27AE60', bg: 'E8F8F0' },
  { shape: 'CROSS',          color: '27AE60', bg: 'E8F8F0' },
  { shape: 'OVAL',           color: 'E8893C', bg: 'FFF3EB' }
];

// ============ 辅助函数 ============
// 技巧：立体阴影 - 向右后方的渐隐投影
const makeShadow = () => ({ type: "outer", blur: 8, offset: 3, color: "000000", opacity: 0.15, angle: 135 });

// 技巧：纸张阴影 - 纸张折叠式的立体阴影
const makePaperShadow = () => ({ type: "outer", blur: 12, offset: 4, color: "000000", opacity: 0.2, angle: 135 });
...
function addCard(slide, x, y, w, h, options = {}) {
  ...
  slide.addShape(pres.shapes.ROUNDED_RECTANGLE, {
    ...
    shadow: options.shadow || makeShadow(),
    rectRadius: 0.1
  });
...
slide1.addShape(pres.shapes.OVAL, {
  x: -2, y: 3, w: 5, h: 5,
  fill: { color: colors.data, transparency: 92 }
});

// 装饰性齿轮（背景纹理）
slide1.addShape(pres.shapes.GEAR_6, {
  x: 7, y: 0.5, w: 3.5, h: 3.5,
  fill: { color: colors.white, transparency: 93 }
});
slide1.addShape(pres.shapes.GEAR_9, {
...
// 左侧卡片 - 行业背景
addCard(slide3, 0.5, 1.5, 4.3, 3.5, { accent: colors.accent, shadow: makePaperShadow() });
...
// 右侧卡片 - 研究意义
addCard(slide3, 5.2, 2.6, 4.3, 2.4, { accent: colors.data, shadow: makePaperShadow() });
...
// 国外研究
addCard(slide4, 0.5, 1.5, 4.3, 3.5, { shadow: makePaperShadow() });
...
// 国内研究
addCard(slide4, 5.2, 1.5, 4.3, 3.5, { shadow: makePaperShadow() });
...
funcReqs.forEach((req, idx) => {
  ...
  // 卡片背景
  addCard(slide5, xPos, 1.5, 2.8, 3.5, { shadow: makePaperShadow() });
...
// 表格背景
addCard(slide7, 0.5, 1.5, 5.5, 3.5, { shadow: makePaperShadow() });
...
frontendFeatures.forEach((feat, idx) => {
  ...
  // 卡片
  addCard(slide9, xPos, yPos, 4.3, 1.5, { accent: feat.color, shadow: makePaperShadow() });
...
backendFeatures.forEach((feat, idx) => {
  ...
  // 卡片
  addCard(slide10, xPos, yPos, 4.3, 1.7, { accent: feat.color, shadow: makePaperShadow() });
...
// 表格背景
addCard(slide11, 0.5, 1.5, 5.0, 3.2, { shadow: makePaperShadow() });
...
// 工作总结
addCard(slide12, 0.5, 1.5, 4.3, 3.5, { accent: colors.accent, shadow: makePaperShadow() });
...
// 未来展望
addCard(slide12, 5.2, 1.5, 4.3, 3.5, { accent: colors.data, shadow: makePaperShadow() });
...
// 保存文件
pres.writeFile({ fileName: "F:/Code/Java/cutting-system/docs/ppt/毕业设计答辩-柜门板材切割排版系统-增强版.pptx" })
  .then(() => {
    console.log("增强版PPT创建成功！");
  })
  .catch(err => {
