...
> 编写日期: 2026-05-19 | 状态: MVP 0-3 已实现，MVP 4 待实施
...
在现有柜门板材切割排版系统中新增 **3D 柜体设计** 功能。用户通过 Three.js 在浏览器中搭建柜体三维模型，一键拆单后自动提取板件切割尺寸、封边信息、铰链孔位，写入订单明细并进入现有排样流程。

**核心价值：** 打通"柜体设计 → 拆单 → 排样"全链路，替代传统的手工测量录入。
...
| 模块 | 职责 |
|------|------|
| `CabinetDesignView.vue` | 3D 设计页主容器，路由 `/cutting/cabinet-design` |
| 左侧面板组件 | 预设列表 + 板件库拖拽源 |
| 中央画布组件 | Three.js 场景渲染与交互 |
| 右侧面板组件 | 选中板件属性编辑 + 柜体全局信息 |
...
```
选择预设(衣柜) → 参数向导(宽高深/层板数/门板数) →
系统程序化生成3D模型 → 3D视图微调 → [一键拆单] →
拆分明细预览弹窗 → 确认写入订单 → 跳转排版工作台
```

**自由拼装（辅助流程）：**

```
从板件库拖拽类型到画布 → 放置定位(吸附对齐) →
调整尺寸/封边 → [一键拆单] → ...
```
...
页面顶部显示订单信息区（订单编号、客户名称），用户在已有订单下进行柜体设计和拆单。若未携带 `orderId`，提示用户先从加工数据输入页进入。
...
```
柜体JSON → ①板材校验 → ②封边尺寸换算 → ③铰链孔位计算 → ④工件编号生成 → 输出
```
...
- 编号范围限定在订单内：追加拆单时从该订单现有最大序号继续
- 示例：订单已有 ZG-001~003，追加新柜体时从 ZG-004 开始
- `splitBatchCode`（批次编号，如 `ZG-20260519-001`）用于标识一次拆单写入的完整批次
...
入参与 confirm 使用同一请求结构（不含 `orderId` 和 `confirmMode`）：
...
入参为柜体 JSON（不是前端传回的明细列表，防止篡改切割尺寸）：
...
- `confirmMode`：第一版仅支持 `append`（追加一个柜体的拆单明细）
- 一个订单可包含多个柜体，每次 `append` 追加新一组拆单明细
...
`confirm` 返回 `nextAction: "layout-workbench"` 后，前端跳转：
...
| 前端状态 | Pinia store (`useCabinetDesign`) | 管理柜体结构、选中状态、撤销栈 |
...
| **MVP 1** | 预设衣柜/地柜参数向导 → 3D 预览 → 拆单 → 写入订单 | 选择预设→输入参数→生成3D模型→点击拆单→TOrderItem/cabinet_order_item 正确写入 |
| **MVP 2** | 拆单后自动衔接排版工作台 | `confirm` → `nextAction` → 跳转排版工作台 → 自动分组提交算法 → Canvas 渲染排样结果 |
| **MVP 3** | 用户模板保存/复用 | 存为模板 → 模板列表可见 → 从模板创建新设计 → 仅可编辑/删除自己的模板 |
| **MVP 4** | 自由拼装、吸附对齐、更多柜型 | 从板件库拖拽 → 3D 吸附放置 → 微调属性 → 拆单 |
...
- 浏览器检查：Three.js canvas 非空、模板生成可见、拆单主流程可走通
...
- **`AGENTS.md`**：
  ...
  - 前端约定新增 `CabinetDesignView.vue`、路由 `/cutting/cabinet-design`、`three` npm 依赖
